...
The pip installation of python-Levenshtein requires g++. After the pip installation all of the build dependencies are removed from the image. The result is a lightweight image of Python and python-Levenshtein.
...
# Create final image from Python 3 + Levenshtein on Alpine Linux
...
```
docker image build --build-arg PYTHON_VERSION=3.10 . -t python-lev:3.10-alpine3.15
```
...
```
docker image build --build-arg PYTHON_VERSION=3.10 . -f Dockerfile-slim -t python-lev:3.10-slim-bullseye
```
